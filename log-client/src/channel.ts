...
export default function initChannel(conn: any, props: any): Promise<any> {
  ...
  if (channelInitializing) {
    return new Promise((resolve) => {
      const timer = setTimeout(() => {
        clearTimeout(timer);
        return resolve(initChannel(conn, props));
      }, 100);
    });
  }
  ...
  const { logExchange } = props;

  return connection.createChannel()
    .then(setChannel)
    .then((ch: any) => {
      console.log('[INFO] Channel for Log Client is created - %s', new Date());
      ch.assertExchange(logExchange, 'direct', { durable: false });
      channelInitializing = false;
